...
# Securely track if we are viewing the main application or the standalone chat window
query_params = st.query_params

if query_params.get("view") == "chat":
    # -------------------------------------------------------------------------
    # STANDALONE DETACHED WINDOW CONTAINER FOR WAYNE-AI
    # -------------------------------------------------------------------------
    st.markdown("""
        <style>
    ...
    # Initialize separate or global session state context for independent window messaging
    if "detached_chat_history" not in st.session_state:
        st.session_state.detached_chat_history = [
            {"role": "assistant", "content": "Hello! I am Wayne-AI. I am running in a dedicated window now. How can I help you analyze your TxDOT metrics today?"}
        ]
    ...
    if user_input:
        st.session_state.detached_chat_history.append({"role": "user", "content": user_input})
        ...
        st.session_state.detached_chat_history.append({"role": "assistant", "content": copilot_reply})

    with chat_container:
        for message in st.session_state.detached_chat_history:
    ...
    # ---------- JAVASCRIPT-POWERED POP-UP DETACHED WINDOW LINK ----------
    # This fires window.open with specialized window dimension strings to completely detach Wayne-AI
    st.markdown("""
        <a href="?view=chat" target="wayne_chat" 
           onclick="window.open('?view=chat', 'wayne_chat', 'width=420,height=580,resizable=yes,scrollbars=yes,status=no,location=no,toolbar=no,menubar=no'); return false;" 
           class="custom-popup-trigger">
            💬 Ask Wayne-AI
        </a>
    """, unsafe_allow_html=True)
